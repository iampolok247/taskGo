            ...
            <div class="p-6">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-xl bg-gradient-to-r <?php echo e($method->type === 'mobile_banking' ? 'from-pink-500 to-pink-600' : ($method->type === 'bank' ? 'from-blue-500 to-blue-600' : 'from-gray-500 to-gray-600')); ?> flex items-center justify-center">
                            <span class="text-white font-bold"><?php echo e(strtoupper(substr($method->name, 0, 2))); ?></span>
                        </div>
                        <div>
                            <h3 class="font-semibold text-gray-900"><?php echo e($method->name); ?></h3>
                            <p class="text-sm text-gray-500 capitalize"><?php echo e(str_replace('_', ' ', $method->type)); ?></p>
                        </div>
                    </div>
                    <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full <?php echo e($method->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'); ?>">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between py-2 border-b">
                        ...
                        <span class="font-medium">৳<?php echo e(number_format($method->min_amount, 2)); ?></span>
                    </div>
                    <div class="flex justify-between py-2 border-b">
                        <span class="text-gray-500">Max Amount</span>
                        <span class="font-medium"><?php echo e($method->max_amount ? '৳' . number_format($method->max_amount, 2) : 'Unlimited'); ?></span>
                    </div>
                    <div class="flex justify-between py-2 border-b">
                        <span class="text-gray-500">Fixed Fee</span>
                        <span class="font-medium">৳<?php echo e(number_format($method->fee_fixed, 2)); ?></span>
                    </div>
                    <div class="flex justify-between py-2">
                        <span class="text-gray-500">Fee %</span>
                        <span class="font-medium"><?php echo e($method->fee_percentage); ?>%</span>
                    </div>
                </div>

                <?php if($method->instructions): ?>
                <div class="mt-4 p-3 bg-gray-50 rounded-lg">
                    <p class="text-sm text-gray-600"><?php echo e($method->instructions); ?></p>
                </div>
                <?php endif; ?>

                <div class="mt-4 flex items-center gap-2">
                    <button onclick="editMethod(<?php echo e($method->id); ?>, '<?php echo e($method->name); ?>', <?php echo e($method->min_amount); ?>, <?php echo e($method->max_amount ?? 'null'); ?>, <?php echo e($method->fee_fixed); ?>, <?php echo e($method->fee_percentage); ?>, '<?php echo e(addslashes($method->instructions ?? '')); ?>', <?php echo e($method->is_active ? 'true' : 'false'); ?>)" class="flex-1 py-2 text-center border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition-all">
                        Edit
                    </button>
                    <form action="<?php echo e(route('admin.settings.payment-methods.toggle', $method)); ?>" method="POST" class="flex-1">
                        <?php echo csrf_field(); ?>
                        <?php echo method_field('PATCH'); ?>
                        <button type="submit" class="w-full py-2 text-center <?php echo e($method->is_active ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'); ?> font-medium rounded-xl hover:opacity-80 transition-all">
...
<div id="addMethodModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl w-full max-w-lg">
        ...
        <form action="<?php echo e(route('admin.settings.payment-methods.store')); ?>" method="POST" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/taskisfk/public_html/resources/views/admin/settings/payment-methods.blade.php ENDPATH**/ ?>