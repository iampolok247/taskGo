...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-gray-500">Review and process user deposit requests</p>
    </div>
    ...
    <!-- Filters -->
    <div class="bg-white rounded-xl p-4 shadow-sm">
        <form action="<?php echo e(route('admin.deposits.index')); ?>" method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php $__currentLoopData = $deposits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $deposit): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center">
                                            <span class="text-primary-600 font-bold text-sm"><?php echo e(strtoupper(substr($deposit->user->name ?? 'U', 0, 2))); ?></span>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900"><?php echo e($deposit->user->name ?? 'Unknown'); ?></div>
                                            <div class="text-sm text-gray-500"><?php echo e($deposit->user->email ?? ''); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">৳<?php echo e(number_format($deposit->amount, 2)); ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php echo e($deposit->paymentMethod->name ?? 'Unknown'); ?>

                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php echo e($deposit->transaction_id ?? 'N/A'); ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php echo e($deposit->created_at->format('M d, Y h:i A')); ?>

                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <?php if($deposit->proof_image): ?>
                                            <a href="<?php echo e(asset('storage/' . $deposit->proof_image)); ?>" target="_blank" class="text-blue-600 hover:text-blue-900">View Proof</a>
                                        <?php endif; ?>
                                        <?php if($deposit->status == 'pending'): ?>
                                            <form action="<?php echo e(route('admin.deposits.approve', $deposit)); ?>" method="POST" class="inline">
                                                <?php echo csrf_field(); ?>
                                                <button type="submit" class="text-green-600 hover:text-green-900">Approve</button>
                                            </form>
                                            <button type="button" onclick="openRejectModal(<?php echo e($deposit->id); ?>)" class="text-red-600 hover:text-red-900">Reject</button>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
    ...
    <?php else: ?>
        <div class="bg-white rounded-xl p-8 text-center shadow-sm">
            ...
            <h3 class="text-lg font-semibold text-gray-900 mb-2">No Deposits Found</h3>
            <p class="text-gray-500">No deposits match your search criteria.</p>
        </div>
    <?php endif; ?>
</div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/taskisfk/public_html/resources/views/admin/deposits/index.blade.php ENDPATH**/ ?>