    ...
    <div class="bg-white rounded-xl p-4 shadow-sm">
        ...
        <form action="{{ route('agent.deposits.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div id="account-info" class="hidden p-4 bg-emerald-50 rounded-xl">
                ...
                <p class="text-xs text-emerald-600 mt-2">Copy these details and send your payment</p>
            </div>
            ...
            <button type="submit" class="w-full py-3 bg-emerald-500 text-white font-semibold rounded-xl hover:bg-emerald-600 transition-all active:scale-[0.98]">
...
function showAccountDetails(radio) {
    ...
    try {
        ...
        if (cat === 'mobile_wallet' || (!cat && (d.number || d.type))) {
            if (accType) html += '<p class="text-sm text-emerald-700"><span class="font-medium">Type:</span> ' + accType + '</p>';
            if (mainNum) html += '<p class="text-lg font-bold text-emerald-900 my-1">' + mainNum + '</p>';
            ...
            if (mainNum) html += '<p class="text-sm font-bold text-emerald-900 my-1 break-all">' + mainNum + '</p>';
        } else {
            if (mainNum) html += '<p class="text-lg font-bold text-emerald-900 my-1">' + mainNum + '</p>';
            if (accType) html += '<p class="text-sm text-emerald-700"><span class="font-medium">Type:</span> ' + accType + '</p>';
            if (d.account_name) html += '<p class="text-sm text-emerald-700"><span class="font-medium">Name:</span> ' + d.account_name + '</p>';
