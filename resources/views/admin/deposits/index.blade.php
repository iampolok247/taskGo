...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-gray-500">Review and process freelancer deposit requests</p>
    </div>
        ...
        <form action="{{ route('admin.deposits.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}" 
                    ...
                    placeholder="Search by freelancer name or transaction ID...">
            </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center">
                                            <span class="text-primary-600 font-bold text-sm">{{ strtoupper(substr($deposit->user->name ?? 'U', 0, 2)) }}</span>
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        @if($deposit->proof_image)
                                            <a href="{{ asset('storage/' . $deposit->proof_image) }}" target="_blank" class="text-blue-600 hover:text-blue-900">View Proof</a>
                                        @endif
                                        @if($deposit->status == 'pending')
                                            ...
                                            <button type="button" onclick="openRejectModal({{ $deposit->id }})" class="text-red-600 hover:text-red-900">Reject</button>
                                        @endif
                                    </div>
                                </td>
                            </tr>
