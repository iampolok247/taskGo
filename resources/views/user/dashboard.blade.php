...
<div class="p-4 space-y-4">
    ...
    <div class="bg-gradient-to-br from-primary-500 via-primary-600 to-primary-700 rounded-2xl p-5 text-white shadow-xl relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <div class="grid grid-cols-2 gap-3">
                <a href="{{ route('user.deposits.create') }}" class="flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 rounded-xl py-3 transition-all">
                    <span class="text-lg">📥</span>
                    <span class="font-medium">Deposit</span>
                </a>
                <a href="{{ route('user.withdrawals.create') }}" class="flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 rounded-xl py-3 transition-all">
                    <span class="text-lg">📤</span>
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            ...
            <p class="text-xl font-bold text-gray-900">{{ $stats['tasks_completed'] ?? 0 }}</p>
    ...
    <div class="bg-gradient-to-r from-purple-500 to-indigo-600 rounded-2xl p-4 text-white shadow-lg">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold tracking-wider">{{ auth()->user()->referral_code }}</p>
            </div>
            <button onclick="copyReferralCode('{{ auth()->user()->referral_code }}')" class="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-xl font-medium text-sm transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-4 gap-3">
        <a href="{{ route('user.deposits.create') }}" class="flex flex-col items-center p-3 bg-white rounded-xl shadow-sm border border-gray-100 active:scale-95 transition-transform">
        ...
        <a href="{{ route('user.withdrawals.create') }}" class="flex flex-col items-center p-3 bg-white rounded-xl shadow-sm border border-gray-100 active:scale-95 transition-transform">
        ...
        <a href="{{ route('user.tasks.index') }}" class="flex flex-col items-center p-3 bg-white rounded-xl shadow-sm border border-gray-100 active:scale-95 transition-transform">
        ...
        <a href="{{ route('user.referrals.index') }}" class="flex flex-col items-center p-3 bg-white rounded-xl shadow-sm border border-gray-100 active:scale-95 transition-transform">
            ...
            <div class="bg-white rounded-xl shadow-sm divide-y divide-gray-100">
                @foreach($recentTransactions as $transaction)
                    <div class="p-4 flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center {{ $transaction->amount > 0 ? 'bg-green-100' : 'bg-red-100' }}">
